/**
 * Upserts selective CMS content without deleting unrelated live rows.
 * For target page IDs: replaces their sections (delete then insert) so structure matches export.
 * Does NOT touch leads, admin_users, clients, or other pages.
 */
import fs from "fs";
...
async function importSelective(filePath) {
  ...
  // Replace sections only for exported pages (keeps other pages untouched)
  if (pageIds.length > 0) {
    const placeholders = pageIds.map(() => "?").join(",");
    await pool.query(
      `DELETE FROM page_sections WHERE page_id IN (${placeholders})`,
      pageIds
    );
  }
  ...
  console.log(
    `Selective import OK: ${media.length} media, ${pages.length} pages, ${sections.length} sections, ${seo.length} seo, ${site_settings.length} settings`
  );
  await pool.end();
}
...
const filePath =
  process.argv[2] ||
  path.resolve(__dirname, "../../../backups/selective-content-export.json");

importSelective(filePath).catch((error) => {
