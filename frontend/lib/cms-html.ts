...
function stripSeoMetadataAppendix(html: string): string {
  return html
    .replace(
      /<hr\s*\/?>\s*<h2[^>]*>\s*SEO Metadata[\s\S]*$/i,
      ""
    )
    .replace(
      /<h2[^>]*>\s*SEO Metadata[\s\S]*$/i,
      ""
    )
    .trim();
}
...
/**
 * When TinyMCE stores pasted HTML source as escaped text inside <p> wrappers,
 * unwrap each paragraph and decode entities so real tags (tables, lists, etc.) render.
 */
...
export function sanitizeCmsHtml(html: string): string {
  return repairEscapedCmsHtml(html)
