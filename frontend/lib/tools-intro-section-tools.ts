...
export function mergeToolsIntroSectionToolsData(
  cmsData: ToolsIntroSectionToolsData | null
): ToolsIntroSectionToolsData {
  if (!cmsData) return DEFAULT_TOOLS_INTRO_SECTION_TOOLS;
  return normalizeToolsIntroSectionToolsData(cmsData);
}

export function toolsIntroPillImageSrc(pill: ToolsIntroPill): string {
  if (pill.image_url) return resolveCmsMediaUrl(pill.image_url);
  return pill.image_src;
}
