...
export function mergeCaseStudySectionData(
  cmsData: CaseStudySectionData | null | undefined
): CaseStudySectionData {
  if (!cmsData) return DEFAULT_CASE_STUDY_SECTION;
  return normalizeCaseStudySectionData(cmsData);
}

export function resolveCaseStudyImage(item: CaseStudyItem): string {
  return item.image_url || item.image_src || "/uploads/tagrobotech.png";
}
