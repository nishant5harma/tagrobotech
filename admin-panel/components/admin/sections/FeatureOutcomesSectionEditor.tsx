...
import {
  normalizeFeatureOutcomesSectionData,
  featureOutcomesSectionToPayload,
...
export default function FeatureOutcomesSectionEditor({
  initialData,
  isActive,
  onIsActiveChange,
  onSave,
}: Props) {
  const [form, setForm] = useState<FeatureOutcomesSectionData>(() =>
    normalizeFeatureOutcomesSectionData(initialData)
  );
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      await onSave(featureOutcomesSectionToPayload(form), isActive);
    } catch (err) {
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          onClick={() =>
            setForm((p) => ({
              ...p,
              items: [...p.items, { value: "", label: "", description: "" }],
            }))
          }
        >
