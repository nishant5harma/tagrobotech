...
export default function TechnologyPlatformSectionEditor({
  initialData,
  isActive,
  onIsActiveChange,
  onSave,
}: TechnologyPlatformSectionEditorProps) {
  const [data, setData] = useState<TechnologyPlatformSectionData>(() =>
    normalizeTechnologyPlatformSectionData(initialData)
  );
  const [openItemIndex, setOpenItemIndex] = useState<number | null>(null);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  function updateField<K extends keyof TechnologyPlatformSectionData>(
    key: K,
    value: TechnologyPlatformSectionData[K]
  ) {
    setData((prev) => ({ ...prev, [key]: value }));
  }

  function updateItem(index: number, patch: Partial<TechnologyPlatformItem>) {
    setData((prev) => ({
      ...prev,
  ...
  function addItem() {
    setData((prev) => ({
      ...
      items: [
        ...prev.items,
        { name: "New platform", description: "", icon: "gps" as PlatformIconKey },
      ],
    }));
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await onSave(technologyPlatformSectionToPayload(data), isActive);
    } catch (err) {
              ...
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-foreground">
                    {index + 1}. {item.name || "Untitled platform"}
                  </p>
                  <p className="truncate text-xs text-muted">
                    {PLATFORM_ICON_OPTIONS.find((option) => option.value === item.icon)?.label ??
                      item.icon}
                  </p>
                </div>
                {isOpen ? (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        value={item.icon}
                        onChange={(e) =>
                          updateItem(index, { icon: e.target.value as PlatformIconKey })
                        }
                        className={inputClass}
                      >
