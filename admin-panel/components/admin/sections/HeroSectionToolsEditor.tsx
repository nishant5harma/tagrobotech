...
export default function HeroSectionToolsEditor({
  initialData,
  isActive,
  onIsActiveChange,
  onSave,
}: HeroSectionToolsEditorProps) {
  const [data, setData] = useState<HeroSectionToolsData>(() =>
    normalizeHeroSectionToolsData(initialData)
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  function updateField<K extends keyof HeroSectionToolsData>(
    key: K,
    value: HeroSectionToolsData[K]
  ) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await onSave(heroSectionToolsToPayload(data), isActive);
    } catch (err) {
