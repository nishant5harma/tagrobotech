...
export default function HeroSectionSoftwareEditor({
  initialData,
  isActive,
  onIsActiveChange,
  onSave,
}: HeroSectionSoftwareEditorProps) {
  const [data, setData] = useState<HeroSectionSoftwareData>(() =>
    normalizeHeroSectionSoftwareData(initialData)
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  function updateField<K extends keyof HeroSectionSoftwareData>(
    key: K,
    value: HeroSectionSoftwareData[K]
  ) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await onSave(heroSectionSoftwareToPayload(data), isActive);
    } catch (err) {
