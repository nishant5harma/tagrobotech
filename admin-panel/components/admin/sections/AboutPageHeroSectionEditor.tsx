...
export default function AboutPageHeroSectionEditor({
  initialData,
  isActive,
  onIsActiveChange,
  onSave,
}: AboutPageHeroSectionEditorProps) {
  const [data, setData] = useState<AboutPageHeroSectionData>(() =>
    normalizeAboutPageHeroSectionData(initialData)
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  function updateField<K extends keyof AboutPageHeroSectionData>(
    key: K,
    value: AboutPageHeroSectionData[K]
  ) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await onSave(aboutPageHeroSectionToPayload(data), isActive);
    } catch (err) {
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="tel:+919319013339 or /contact"
              className={inputClass}
            />
          </div>
        </div>
