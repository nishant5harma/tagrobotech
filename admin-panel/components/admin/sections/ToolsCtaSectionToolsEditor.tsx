...
import {
  ...
  toolsCtaSectionToolsToPayload,
  type ToolsCtaSectionToolsData,
} from "@/lib/tools-cta-section-tools";

type ToolsCtaSectionToolsEditorProps = {
  initialData: unknown;
  isActive: boolean;
...
export default function ToolsCtaSectionToolsEditor({
  initialData,
  isActive,
  onIsActiveChange,
  onSave,
}: ToolsCtaSectionToolsEditorProps) {
  const [form, setForm] = useState<ToolsCtaSectionToolsData>(() =>
    normalizeToolsCtaSectionToolsData(initialData)
  );
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await onSave(toolsCtaSectionToolsToPayload(form), isActive);
    } catch (err) {
